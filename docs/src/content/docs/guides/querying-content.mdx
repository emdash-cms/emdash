...
For collections with `supports: ["seo"]`, editors can set an SEO title, meta description, OG image, canonical URL, and a "hide from search engines" (noindex) toggle in the admin's SEO panel. That data is delivered on the entry as `entry.data.seo`. On server-rendered pages that include the `<EmDashHead>` component and fetch their entry through `getEmDashEntry()`, the panel values are applied to the rendered head automatically — description, image, canonical, and noindex need no template wiring, and the panel title feeds `og:title`/`twitter:title`. The `<title>` element, prerendered pages, and templates that don't render `<EmDashHead>` still use `getSeoMeta`, which resolves the panel fields (with sensible fallbacks to `data.title` / `data.excerpt`) into ready-to-render meta tags:
...
<Aside type="caution">
	...
	`<EmDashHead>` or `getSeoMeta` (or read the raw fields via `getContentSeo`).
</Aside>
...
Inline code blocks follow the system's light or dark appearance by default. If your site has its own theme switch, set these properties in its light and dark theme selectors:

| Property | Purpose |
| --- | --- |
| `--emdash-inline-code-background` | Code block background |
| `--emdash-inline-code-foreground` | Plain code text |
| `--emdash-inline-code-muted` | Comments and quoted text |
| `--emdash-inline-code-keyword` | Keywords, literals, selectors, and deleted text |
| `--emdash-inline-code-string` | Strings, attributes, symbols, and added text |
...
The following example styles inline code blocks for a site that applies `.dark` to the `<html>` element:
...
If you change the code block background, also override every text color so the syntax remains readable. These properties apply only while inline editing is active. Style published code blocks with your site's CSS.
