...
Open a local media item's details to see which EmDash-managed entries reference it. References that EmDash has already found remain visible while existing content is still indexing.

Administrators see a setup notice until historical indexing is ready. Open **Settings → Media Usage**, select **Enable Media Usage**, then select **Turn on**. Keep that page open while it indexes existing content. If you leave, return to continue from the stored progress.
