...
2. Select **Upload to Library** to open the upload dialog, then select **Browse files** or drop files on the dialog. You can also drag files anywhere over the media library and drop them on the upload overlay.

3. Monitor each file in the upload dialog. Uploads start when you select or drop the files, with up to three files uploading at once.

4. Retry failed files or cancel files that are queued or uploading. Select **Done** after the unfinished uploads complete or are canceled.
