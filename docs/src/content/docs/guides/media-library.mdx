...
Incremental Media Usage capture is staged but deliberately inactive in this release. Normal saves
do not create or process durable indexing work until activation becomes available in a later
safety-complete release. Use collection-scoped Media Usage repair to refresh the index in the
meantime.

After activation, EmDash records durable work when a supported content save changes its media
references. It tries the work immediately and, when scheduled maintenance is enabled, retries
transient failures in bounded background runs. Completed work is removed from the queue.
...
replace the active worker.

<Aside type="caution">
	While incremental capture is inactive, use collection-scoped Media Usage repair after imports,
	direct database writes, or any suspected gap. After activation, disabling scheduled maintenance
	still allows successful saves to attempt immediate indexing, but a failed job has no automatic
	freshness deadline. The work list is not a historical scan, and an empty page alone does not
	prove complete historical coverage.
</Aside>
