...
Archives are stored under the `backups/` prefix in your bucket as `emdash-backup-<timestamp>.json`. The **Stored Backups** list in the admin lets you download or delete individual archives, and **Back up now** creates one on demand.
