...
For an offline SQLite backup, stop every process that writes the database and copy the database file together with any `-wal` and `-shm` files next to it. The `-wal` file can hold committed changes that are not yet in the main file. For a consistent online backup, use SQLite's backup command:
