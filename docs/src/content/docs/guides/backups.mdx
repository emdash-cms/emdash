...
To copy a site's content, settings, and media into a new EmDash site, including one on a different database, use a [site package](/guides/site-transfer/) instead.

If the site stores encrypted plugin settings, keep the complete `EMDASH_ENCRYPTION_KEY` rotation list in a separate secret-manager backup. The key is never stored in the database, D1 Time Travel, a SQL dump, or a JSON export.
...
- Site settings such as the title, tagline, URL, locale, logo, display preferences, social profiles,
  and SEO defaults. These come from the `site:` setting group and the `emdash:site_title`,
  `emdash:site_tagline`, and `emdash:locale` settings.
...
- Media folders, records of where media is used, incomplete or in-progress uploads, and the media
  files themselves
- Other site options, including the deployment URL recorded at setup (`emdash:site_url`), the
  preview signing secret, and the backup schedule
...
The download is for inspection or custom tooling. Before bulk imports, schema changes, or
major upgrades, create a restorable database backup using one of the options below. To move the
site to another EmDash site, export a [site package](/guides/site-transfer/) under **Settings → Transfer**.
...
Time Travel restores the entire database, including content, users, settings, plugin data, and migration records. It does not restore R2 media objects or `EMDASH_ENCRYPTION_KEY`. Before testing plugin integrations, restore every encryption key referenced by the recovered plugin settings. After the command completes, deploy the application version that matches the restored database, reopen traffic, and verify sign-in, content reads, schema changes, an encrypted plugin setting, and a write.
...
Keep the SQL file with the matching application version, the media backup created at the same time, and a separately stored copy of the `EMDASH_ENCRYPTION_KEY` rotation list. Test recovery by importing the dump into a newly provisioned empty D1 database, restoring the runtime keys, updating a non-production binding to that database, and verifying the site.
...
Back up the local upload directory or S3-compatible bucket and the `EMDASH_ENCRYPTION_KEY` rotation list separately. To recover, stop every server process, retain a copy of the damaged database, replace it with the verified backup, restore the runtime keys and any required media objects, and start the matching application version. Verify sign-in, public content, an encrypted plugin setting, an edit, and a media read before reopening traffic.
...
To rebuild a site's content in a new, empty EmDash site, export a [site package](/guides/site-transfer/)
from the original site while it is still available and import it into the new site. A site package
does not restore users, API tokens, plugin data, or secrets, so it does not replace a database
