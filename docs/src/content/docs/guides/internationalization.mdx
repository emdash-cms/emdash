...
At startup, EmDash warns when taxonomy definitions or terms use a locale that
is not in the site's configured `i18n.locales`. Without an `i18n` configuration,
`en` is the effective locale. These rows are left unchanged because EmDash
cannot infer which configured locale the existing content was meant to use.
