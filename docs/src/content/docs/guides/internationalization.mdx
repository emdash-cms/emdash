...
Terms are per-locale. Each locale has its own definition, so `label` and
`labelSingular` can be translated too, while `hierarchical` and `collections`
are shared by every locale (see
[Translate taxonomies and terms](/guides/taxonomies/#translate-taxonomies-and-terms)). The pivot
