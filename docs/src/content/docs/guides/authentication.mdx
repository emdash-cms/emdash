...
EmDash limits the unauthenticated endpoints that begin login, signup, or OAuth client registration. The limits are separate for each endpoint and trusted client IP:
...
| `POST /_emdash/api/oauth/device/code` | 10 requests per minute |
| `POST /_emdash/api/oauth/device/token` | 12 requests per minute |
| `POST /_emdash/api/oauth/register` | 10 registrations per minute |
