...
| `icon` | `string` | Phosphor icon name for the sidebar entry, such as `calendar-blank` |
| `admin.listColumns` | `string[]` | Up to four declared field slugs shown in the content list |
| `admin.quickCreate` | `boolean` | Whether the dashboard shows a "new entry" quick action for this collection; defaults to `true` |
...
| `hidden` | `boolean` | Hides the generated sidebar link, command palette link, and dashboard quick action; the collection stays reachable by URL and API |
...
[`reference`](/reference/field-types/#reference) documents both forms and the validation keys each
one takes.
...
For a reference field, declare the links from the parent end of its relation. Both ends view one set
of links, so a field on the child collection would restate links the parent already carries.
...
- Taxonomy definitions and terms honor the applicable conflict mode. The exception is the built-in `category` and `tag` definitions that every new database starts with: until a site edits them, a seed that declares them replaces them in every mode.
- Settings use per-key conflict handling. `skip` creates missing settings and preserves existing values. `update` overwrites every supplied setting. `error` stops at the first existing setting; settings created earlier in seed order remain applied.
...
`export-seed` works directly on a local SQLite file. For a deployed D1 database, export it to a local file first. Review exported settings, content, and media references before committing the result. To make exported media importable on another site, pass `--media-base-url`; see [Media URLs](/reference/cli/#media-urls).
