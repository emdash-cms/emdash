...
EmDash provides official starter themes, each available in local and Cloudflare variants:
...
| `@emdash-cms/template-service-business` | Lead-focused local business site with services, projects, reviews, FAQs, service areas, team, credentials, and LocalBusiness schema | Contractors, home services, studios, and local professionals |
...
npm create astro@latest -- --template @emdash-cms/template-service-business-cloudflare
```

These variants include `wrangler.jsonc` for deployment configuration. Most use D1 and R2. The service-business variant uses only Workers, D1, and static assets so it can stay entirely within the free tier; R2-backed editor uploads are optional.
