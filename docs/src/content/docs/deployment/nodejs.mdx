...
`EMDASH_ENCRYPTION_KEY` is optional and currently affects startup validation only. EmDash does not use it to encrypt or decrypt stored values, and plugin secrets remain plaintext in the database even when it is set. [Secrets and key management](/deployment/secrets/) explains its format and the effects of rotating or losing each secret.
