...
| `revalidate` | `number` | `1000`   | Cross-isolate staleness window, in milliseconds. See [Freshness](#freshness).     |
...
| `revalidate` | `number` | `1000`  | Staleness window for cached collection versions, in ms.|
...
<Aside type="note">
Authenticated requests, preview links, and visual editing always bypass the object cache, so editors see the current content immediately.
</Aside>

For anonymous visitors, a change can take up to `revalidate` milliseconds (default one second) to appear across all isolates. Lower `revalidate` for faster propagation at the cost of more reads against the cache; raise it to read the cache less often.
