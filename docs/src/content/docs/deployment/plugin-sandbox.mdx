...
The runner depends on the deployment platform. On Cloudflare Workers, each plugin runs as a [Dynamic Worker](https://developers.cloudflare.com/dynamic-workers/) created through the Worker Loader binding. On Node.js, the server starts [`workerd`](https://github.com/cloudflare/workerd), the open-source Workers runtime, as a child process and runs each plugin as a service inside it. The `sandboxRunner` option of `emdash()` selects the runner and enables the hosted registry catalog. Without it, plugins under `sandboxed: []` are not loaded. An explicitly configured registry remains browseable, but installing or updating a sandboxed plugin fails with `SANDBOX_NOT_AVAILABLE`.
...
Each runner applies resource limits per plugin hook or route invocation. Set `sandboxLimits` in `emdash()` to override one or more defaults for every sandboxed plugin on the site.

| Limit       | Default | Cloudflare Workers                                                     | Node.js                |
| ----------- | ------- | ---------------------------------------------------------------------- | ---------------------- |
| CPU time    | 50 ms   | Enforced by the Worker Loader; the plugin throws when it hits the limit | Not enforced           |
| Subrequests | 30      | Enforced by EmDash and Worker Loader; the plugin throws at the limit    | Not enforced           |
...
The following example lowers the subrequest budget and shortens the wall-time limit:
...
On Cloudflare, each bridge-backed context operation counts as a subrequest. This includes storage, KV, settings, content, network, and log calls. EmDash counts these calls in the plugin wrapper so local `@emdash-cms/plugin-test` runs enforce the same configured budget. Worker Loader also enforces the limit after deployment. Local helpers such as `ctx.url()` and values such as `ctx.site` do not cross the bridge. Use storage batch methods such as `getMany()`, `putMany()`, and `deleteMany()` when one invocation handles several records.

Cloudflare limits one request chain to [32 Worker invocations](https://developers.cloudflare.com/workers/runtime-apis/bindings/service-bindings/#limits), and every Service Binding call counts toward that total. A request normally enters the host Worker and the plugin Dynamic Worker before the plugin calls the bridge, so the default leaves room for 30 bridge calls. Raising `subrequests` above 30 does not remove Cloudflare's request-chain limit.

The Node.js runner logs a warning when `sandboxLimits` contains `cpuMs`, `memoryMb`, or `subrequests`, because standalone workerd cannot enforce those settings. It still enforces `wallTimeMs`.
