...
A selected runner can still be unavailable at runtime: on Cloudflare Workers when the deployed `LOADER` binding or `PluginBridge` export is missing, and on Node.js when `workerd` is not installed or its binary does not run. EmDash then logs the following warning when the runtime starts:
...
On Node.js, run the installed binary directly so the check cannot download a missing package:
