...
| -------------------------------------------------- | --------------------------------- | ---------------------------------------- | ----------------------------------------------- |
| [`EMDASH_ENCRYPTION_KEY`](#the-encryption-key)     | Operator (`emdash secrets generate`) | Environment / Worker secret only         | Encrypted plugin settings cannot be read until the matching key is restored |
...
| [Plugin secrets](#plugin-secrets)                  | You (admin settings UI)           | Encrypted database setting               | Restore the matching encryption key or re-enter the value |
...
`EMDASH_ENCRYPTION_KEY` encrypts plugin settings declared with `type: "secret"`. EmDash uses AES-GCM with the plugin ID and setting key as authenticated data. A malformed value produces an operator-facing startup message, and operations that need encrypted plugin settings fail closed. Unrelated site requests continue to work.
...
The format is `emdash_enc_v1_` followed by 32 random bytes as unpadded base64url. The value is operator-provided and is not stored in the database. Keep it in a secret manager and in a separate recovery backup.

<Aside type="caution">
	A database backup does not contain `EMDASH_ENCRYPTION_KEY`. Restoring the database without every
	key referenced by its encrypted plugin settings leaves those values unreadable.
</Aside>

To rotate the key, prepend a new value and retain the old value after a comma:

```text title="Runtime secret"
EMDASH_ENCRYPTION_KEY=emdash_enc_v1_<new-key>,emdash_enc_v1_<old-key>
```

EmDash encrypts new and re-saved values with the first key. It uses the stored `kid` fingerprint to select an older key for reads. Re-save every plugin secret before removing the old key, then verify those integrations from a deployment that contains only the new key. EmDash does not currently report which key IDs remain in use, so keep an inventory of the credentials you resave and do not remove an old key until each integration has passed that verification.
...
- **Sessions** use Astro's session store (Workers KV on Cloudflare, filesystem on Node). Adapters that do not configure a store, such as Vercel, need a [session driver](https://docs.astro.build/en/guides/sessions/#configuring-sessions) in `astro.config.mjs`; without one, sign-in fails with an error explaining that a session driver is required. The cookie carries an opaque session ID; there is no signing secret to manage. Sign out to end a session, or clear the session store (e.g. the KV namespace) to force everyone to sign in again.
...
Settings a plugin declares with `type: "secret"` (API keys for email providers, form CAPTCHAs, etc.) are entered in the admin UI and encrypted in the `options` table under `plugin:<id>:settings:<key>`. The admin receives only whether a secret is set, even when the matching encryption key is unavailable, so an administrator can replace an unreadable credential. Plugin code reads the plaintext through `ctx.settings` inside its isolated runtime. Values stored outside a declared settings schema, including arbitrary plugin KV and state entries, do not use this encryption path.

<Aside type="caution">
	Encryption at rest does not limit what the installed plugin can do with a value you submit to its
	settings form. Review the plugin's declared network and email access, and prefer scoped, revocable
	credentials.
</Aside>

- **Credential rotation:** rotate the credential at the provider and paste the new value into the plugin's settings page. The save writes a new encrypted envelope.
- **Plaintext migration:** secrets stored by an earlier EmDash release remain readable. Save each value again to encrypt it.
- **If the encryption key is lost:** restore the matching `EMDASH_ENCRYPTION_KEY` from the separate key backup. If no copy exists, replace every affected credential at its provider and enter the replacements after configuring a new encryption key.
...
| I want to…                              | Do this                                                                        |
| --------------------------------------- | ------------------------------------------------------------------------------ |
| Rotate plugin-setting encryption        | Prepend the new key, re-save plugin secrets, then remove the old key           |
