...
Cloudflare runs scheduled publishing, plugin tasks, and general maintenance from one Cron Trigger. Media Usage setup advances through requests from the visible Settings page.
...
3. Keep the Settings page open until it shows **Ready**. If you close or hide it, return to the page to continue from the stored progress.
4. Resume direct D1 writers when the page shows **Indexing existing content** or **Ready**.

EmDash may briefly pause editing while the page shows **Setting up**. If the page shows **Needs attention**, follow its recovery guidance. **Retry setup** appears only after activation fails.
