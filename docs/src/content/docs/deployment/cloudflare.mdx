...
That's the whole setup: EmDash invalidates cached entries automatically when content changes (see [Edge Caching](/deployment/edge-cache/) for the tag convention). You only need to purge yourself for custom tags — for example a manual "clear everything" switch, using the platform's [cache purge API](https://developers.cloudflare.com/workers/cache/) from your Worker (no Cloudflare REST credentials needed).
...
For a deeper guide — route rules, cache tags, EmDash's built-in purge-on-publish, and the responses that must stay out of the cache — see [Edge Caching](/deployment/edge-cache/).
