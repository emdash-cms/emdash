...
Set these values as Worker secrets or environment variables:

| Variable          | Example                         | Notes                                                    |
| ----------------- | ------------------------------- | -------------------------------------------------------- |
| `EMAIL_SMTP_HOST` | `smtp-relay.brevo.com`          | Any SMTP server reachable from your deployment           |
| `EMAIL_SMTP_PORT` | `587` or `465`                  | 587 uses STARTTLS, 465 uses implicit TLS. Port 25 is refused. |
...
<Aside type="caution">
	Cloudflare blocks outbound port 25. The transport supports only submission ports 587 (STARTTLS) and 465 (implicit TLS), and always requires TLS before authenticating.
</Aside>

When configured, the SMTP transport registers as a built-in email provider. A plugin provider you select under **Settings → Email** (such as `cloudflareEmail()`) takes precedence; the SMTP provider is used when it is the only provider or when you select it there.
