...
`EMDASH_ENCRYPTION_KEY` will encrypt plugin secrets at rest (webhook
tokens, Turnstile keys, etc.) once plugin secret encryption ships in a
forthcoming release. PR 1 of the secrets work parses and validates the
key; the actual encrypt/decrypt at the plugin storage layer ships in a
follow-up. Setting it now means your deployment is ready when the
feature lands. Until then, plugin secrets continue to be stored as
plaintext.

The key is operator-provided. There is no database fallback — it never
touches the database, only encrypted ciphertext does. Losing the key
means losing every secret encrypted with it once encryption ships.
