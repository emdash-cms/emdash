...
The plugin uses the binding named `EMAIL` unless its `binding` option names another one. If it is the only active email provider, EmDash selects it automatically. If more than one provider is active, choose the Cloudflare provider under **Settings → Email**. The optional `replyTo` address receives replies without changing the accepted From address. A plugin can set `replyTo` on an individual message, which overrides this option for that message.

### Built-in SMTP transport

If you already have SMTP credentials (for example Brevo relay, Office 365, Fastmail, Amazon SES, or a self-hosted Postfix relay), use the SMTP transport built into EmDash instead of Cloudflare Email Sending. It connects through `cloudflare:sockets` on Workers and `node:net`/`node:tls` on Node.

Configure it under **Settings → Email** in the admin, where the password is stored encrypted with `EMDASH_ENCRYPTION_KEY`, or set these values as Worker secrets or environment variables:

| Variable                | Example                         | Notes                                                                       |
| ----------------------- | ------------------------------- | --------------------------------------------------------------------------- |
| `EMAIL_SMTP_HOST`       | `smtp-relay.brevo.com`          | Any SMTP server reachable from your deployment                              |
| `EMAIL_SMTP_PORT`       | `465` or `587`                  | 465 uses implicit TLS (recommended on Workers), 587 uses STARTTLS. Port 25 is refused. |
| `EMAIL_SMTP_USER`       | `you@example.com`               | SMTP auth username                                                          |
| `EMAIL_SMTP_PASS`       | `xsmtpsib-…`                    | SMTP auth password (store as a secret)                                      |
| `EMAIL_SMTP_SECURE`     | `tls` or `starttls`             | Optional; inferred from the port (465 is `tls`, otherwise `starttls`)       |
| `EMAIL_SMTP_FROM`       | `My Site <noreply@example.com>` | Optional default sender; falls back to `EMAIL_SMTP_USER`                    |
| `EMAIL_SMTP_FROM_NAME`  | `My Site`                       | Optional structured alternative to `EMAIL_SMTP_FROM`, with `EMAIL_SMTP_FROM_EMAIL` |
| `EMAIL_SMTP_FROM_EMAIL` | `noreply@example.com`           | Optional structured alternative to `EMAIL_SMTP_FROM`, with `EMAIL_SMTP_FROM_NAME`  |
| `EMAIL_SMTP_REPLY_TO`   | `hello@example.com`             | Optional Reply-To header                                                    |

<Aside type="caution">
	Cloudflare blocks outbound port 25. The transport supports only submission ports 465 (implicit TLS) and 587 (STARTTLS), and always requires TLS before authenticating. Prefer port 465 on Workers: STARTTLS upgrades over `cloudflare:sockets` are less reliable than implicit TLS.
</Aside>

The SMTP transport registers as a built-in email provider. When these variables are set and no other provider is configured, EmDash selects it automatically; otherwise choose it under **Settings → Email**. An explicitly selected provider, plugin or built-in, always takes precedence.
...
`EMDASH_ENCRYPTION_KEY` encrypts plugin settings declared as secrets. Set it before saving a plugin secret, and preserve it separately from D1 backups. During rotation, provide the new key first and retain older keys after commas until every plugin secret has been saved again. [Secrets and key management](/deployment/secrets/#the-encryption-key) describes rotation and recovery.

The plugin bridge reads this Worker secret binding directly. The generated admin settings route reads it through `process.env`. With `nodejs_compat`, Cloudflare populates `process.env` by default for compatibility dates on or after `2025-04-01`. Projects pinned to an earlier date must also add `nodejs_compat_populate_process_env` before saving encrypted settings.
