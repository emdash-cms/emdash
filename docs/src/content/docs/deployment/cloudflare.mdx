...
Before opening the deployed site, inspect the D1 target and apply its core migrations. Unlike Wrangler, `emdash migrate` does not use the saved local login. It calls the Cloudflare API with `CLOUDFLARE_API_TOKEN` and requires the account ID from `--account-id`, `account_id` in Wrangler configuration, or `CLOUDFLARE_ACCOUNT_ID`; it does not prompt for an account. Replace the example account ID below with the 32-character ID for the account that owns `my-emdash-site`.
...
After confirming the account and database, apply the migrations interactively and verify the result.
...
The token needs D1 Edit permission. Non-interactive jobs also require the reviewed target fingerprint. For later releases, apply migrations before deploying the matching build. Serialize jobs by account and database UUID. [Manage core database migrations](/deployment/core-migrations/) is the canonical runbook for CI, runtime modes, partial failures, and rollback boundaries.
...
Cloudflare runs a Worker near the visitor by default. EmDash server-rendered requests make several D1 round trips, so use [Targeted Placement](https://developers.cloudflare.com/workers/configuration/smart-placement/) when database latency matters more than the extra distance between the visitor and the Worker.

Wrangler accepts `placement.mode: "targeted"` with exactly one selector: `region`, `host`, or `hostname`. Select the value that targets the D1 primary location, and add the resulting `placement` object to `wrangler.jsonc`. Targeted Placement moves Worker execution; it does not enable D1 read replication or change EmDash's `session` setting. Measure response latency after deployment because placing the Worker closer to D1 can place it farther from some visitors.
...
1. Use Astro's Cloudflare cache provider so route rules and `Astro.cache` set cache headers and invalidation uses `cache.purge()`.
   ...
   The `@astrojs/cloudflare` adapter detects `cacheCloudflare()` and enables Workers Cache in the generated deployment configuration. Do not duplicate the generated `cache.enabled` setting in the source `wrangler.jsonc`; inspect `.wrangler/deploy/config.json` after a build when you need to verify it.
...
Build once, inspect and apply migrations to the preview D1 binding, then deploy that build to the preview environment:
