...
This guide uses [Astro's route caching](https://docs.astro.build/en/guides/caching/) with the Cloudflare cache provider, and applies to Cloudflare deployments only. It replaces the `cloudflareCache()` provider from `@emdash-cms/cloudflare`, which is deprecated and kept only for existing sites. The two caches are complementary: the edge cache serves repeat requests, the object cache makes the remaining renders cheap.
...
```astro title="src/pages/blog/[slug].astro"
if (!entry) {
	if (Astro.cache?.enabled) Astro.cache.set({ maxAge: 60, tags: ["pages", "posts"] });
