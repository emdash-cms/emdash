...
Site chrome is covered too: saving site settings purges `emdash:settings`, menu edits purge `emdash:menu:{name}`, and taxonomy and widget-area writes purge their matching `emdash:taxonomy:{name}` / `emdash:widget-area:{name}` tags. Pages pick these tags up automatically when they use the `cacheHint` from `getMenuWithCacheHint()` or `getSiteSettingsWithCacheHint()`.
