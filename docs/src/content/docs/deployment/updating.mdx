...
## Template files are not updated

Creating a site from a template copies the template's files, such as layouts, components, styles, and `astro.config.mjs`, into your project. An update changes the installed packages only and leaves these files as they are. Later changes to a template, for example the move from Google Fonts `<link>` tags to the Astro Fonts API, apply to new sites only. To adopt one in an existing site, compare your files with the current version in the [`templates` directory](https://github.com/emdash-cms/emdash/tree/main/templates) and copy the changes you want.
