...
These grants let the role create new objects. They do not change the owner of existing tables; see [Repair mixed PostgreSQL ownership](#repair-mixed-postgresql-ownership).
...
This does not move an existing installation from `public` or repair mixed ownership. Existing sites should keep their current schema and follow [Repair mixed PostgreSQL ownership](#repair-mixed-postgresql-ownership) instead.
