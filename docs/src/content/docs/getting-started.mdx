...
For production, set an encryption key now to be ready when plugin
secret encryption ships in a forthcoming release:

```bash
npx emdash secrets generate
```

Add the result to your environment as `EMDASH_ENCRYPTION_KEY`. The key
is parsed and validated by the current release but is not yet used to
encrypt anything; the encrypt/decrypt path at the plugin storage layer
ships in a follow-up. Setting it now means your deployment is ready
...
This key never touches the database; only encrypted ciphertext will
once encryption ships. Back it up somewhere durable — losing it means
losing every secret encrypted with it.
