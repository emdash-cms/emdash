...
<Aside type="caution">
	On Cloudflare D1, revision restore for a collection without revision support updates the content
	and writes its audit revision as separate operations. If the request fails after the content update,
	read the entry and its revision history before retrying.
</Aside>
