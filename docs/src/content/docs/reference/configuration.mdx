...
**Optional.** Configures the [plugin registry](/plugins/registry/) aggregator and policy. With no explicit value, EmDash uses `https://registry.emdashcms.com` when [`sandboxRunner`](#sandboxrunner) is configured and `sandbox` is not `false`.

Set `registry: false` to disable registry discovery and registry-installed plugins while keeping the sandbox runner available for plugins declared in `sandboxed` and legacy Marketplace plugins:
...
See [The plugin registry](/plugins/registry/) for the install workflow and trust model.

### `marketplace`

**Deprecated.** Base URL used to update plugins installed from the legacy Marketplace. Marketplace browsing and new installs are not shown in the admin. Existing Marketplace plugins remain updateable and uninstallable while this option is configured.
...
Production URLs must use HTTPS; HTTP is accepted only for `localhost` and `127.0.0.1` during development. Keep this option until every Marketplace plugin has been replaced or uninstalled, then remove it. Follow [Migrate from Marketplace](/plugins/migrate-from-marketplace/) for the complete procedure.
...
The public browser-facing origin for the site (scheme + host + optional port, **no path**). Set it before running production setup. Only loopback development hosts can complete setup without a configured origin.
...
Setup fails with `SITE_URL_REQUIRED` on a non-loopback host when neither source is set. This prevents the first unauthenticated setup request from choosing the origin used in later authentication emails.

On **Cloudflare Workers**, the env-var fallback reads `process.env`. With `nodejs_compat`, Cloudflare populates `process.env` by default for compatibility dates on or after `2025-04-01`. Projects pinned to an earlier date must also add `nodejs_compat_populate_process_env`.
...
If your proxy writes a client-IP header, set [**`trustedProxyHeaders`**](#trustedproxyheaders) so EmDash's rate limits can use the real client IP. Without a trusted client IP, the authentication rate limits are skipped and all public comment submissions share one rate-limit bucket (20 comments per 10 minutes across all visitors).
...
**Optional.** Headers to trust for client-IP resolution when running behind a reverse proxy you control. Used by auth rate limits (magic-link, signup, passkey, OAuth device flow, OAuth client registration) and the public comment endpoint.

On Cloudflare the `cf` object attached to the request is used automatically — you normally do **not** need to set this. On self-hosted deployments behind nginx, Caddy, Traefik, Fly, Railway, or similar, set this to the header your proxy writes so rate limits can count requests per real client IP.
...
<Aside type="caution">
	**Only set this when you control the reverse proxy.** Untrusted clients can set any header
	...
	proxy in front, leave this unset rather than trust a spoofable header. EmDash then skips the
	authentication rate limits, counts all public comment submissions in one shared bucket, and
	treats all visitors as one voter for comment reactions.
</Aside>
...
**Deprecated.** Use the top-level [`registry`](#registry) option. Existing `experimental.registry` configuration continues to work when the top-level option is omitted. If both are present, the top-level value takes precedence.

The following change moves an existing registry URL to the top level:

```js title="astro.config.mjs" del={2-4} ins={5}
emdash({
	experimental: {
...
Each `*EnvVar` option below names an environment variable. The provider reads it in this order: a Cloudflare Workers binding of that name, then `process.env` on the Node adapter. The matching direct option (`accountId`, `accountHash`, `apiToken`) always takes precedence over both.
