...
**Optional.** Customize the [plugin registry](/plugins/registry/) aggregator and policy. With no explicit value, EmDash uses `https://registry.emdashcms.com` when [`sandboxRunner`](#sandboxrunner) is configured and `sandbox` is not `false`.

An explicit value displays registry browsing without a sandbox runner, but installing or updating a registry plugin still requires an available runner because registry plugins run sandboxed.
