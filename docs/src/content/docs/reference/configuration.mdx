...
**Optional.** Sets resource limits for each sandboxed plugin hook or route invocation. Omitted values use the runner defaults.

The following example gives each invocation 75 milliseconds of CPU time, 20 subrequests, and 15 seconds of wall time:
...
Cloudflare Worker Loader enforces `cpuMs`. EmDash and Worker Loader enforce `subrequests`. Both runners enforce `wallTimeMs`. The Node.js runner accepts `cpuMs`, `memoryMb`, and `subrequests` for configuration portability but cannot enforce them. See [Plugin Sandbox: Resource limits](/deployment/plugin-sandbox/#resource-limits) for defaults and platform ceilings.
