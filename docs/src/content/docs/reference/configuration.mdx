...
Pass a bare aggregator URL string, or an object when you need a labeler or a release-age policy. The following example uses the object form:
...
| `acceptLabelers`                  | `string`               | Comma-separated labeler DIDs forwarded to the aggregator for takedown and verification labels. |
