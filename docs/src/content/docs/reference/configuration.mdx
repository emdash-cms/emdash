...
Set `EMDASH_ALLOWED_ORIGINS` to a comma-separated list of additional accepted origins. The canonical `siteUrl` remains the source of `rpId`, and the listed origins are accepted at verification time:
...
Each entry must be a valid `http:` or `https:` URL; entries are reduced to their origin (path/query stripped). Each origin must share the registrable parent domain with `siteUrl` -- this is a WebAuthn requirement, not an EmDash one.
