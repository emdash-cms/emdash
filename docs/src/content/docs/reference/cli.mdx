...
Scheduled entries are exported as drafts, because a seed has no field for a publish time. Redirect rules with status 410 or 451 are left out with a warning on stderr.
