...
The content API and the admin editor reject anything else, including protocol-relative URLs (`//example.com`) and schemes that can run script, such as `javascript:` and `data:`.
...
URL fields are stored as text. Seeds and plugin content updates are not format-checked. They still reject a value that uses any scheme other than `http:`, `https:`, `mailto:`, or `tel:`.

Entries saved by earlier EmDash releases can hold a `url` value with some other scheme. Those values stay stored and queries return them unchanged. A write that sends the value again is rejected until the value is corrected. This includes an admin save of that entry and duplicating it. To render such values safely, pass `url` values through [`sanitizeHref()`](/reference/api/#sanitizehref-and-issafehref) before putting them in an `href`.
