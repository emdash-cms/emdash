...
EmDash includes a built-in moderator that applies the collection's comment settings. When a site has no stored moderator choice and exactly one plugin provides `comment:moderate`, EmDash selects the plugin. When several plugins provide it, EmDash selects none and new comments stay pending. EmDash stores the choice it makes, so a site that started without a moderation plugin keeps the built-in moderator after one is added.

An administrator changes the choice with an API token that has the `admin` scope. The following request selects a plugin by its ID:
