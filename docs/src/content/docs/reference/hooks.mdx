...
EmDash includes a built-in moderator that applies the collection's comment settings. When exactly one plugin provides `comment:moderate`, that plugin replaces the built-in moderator. When several plugins provide it, EmDash selects none of them and new comments wait for review. A stored moderator choice is kept. Earlier EmDash releases stored the built-in moderator as the choice, so a site that has run one keeps the built-in moderator.
