...
| `content:beforePublish` | Before content is published            | Can cancel        | No        |
| `content:beforeSchedule` | Before content is scheduled           | Can cancel        | No        |
| `content:beforeUnpublish` | Before content is unpublished         | Can cancel        | No        |
...
| `byline:afterSave`      | After a byline is created or updated  | Nothing           | No        |
| `byline:afterDelete`    | After a byline is deleted             | Nothing           | No        |
...
Byline hooks run after the admin API or an MCP tool changes a byline profile. They do not run for seeds or imports. Use them to keep an external copy of author data, such as a search index, current. Hook errors are logged and do not undo the change.

Relinking a byline to a different user can change the credits inferred for that user's entries. No per-entry hook runs for that change.

### `byline:afterSave`

**Capability:** `bylines:read`

Runs after a byline or a byline translation is created or updated.

```ts
hooks: {
  "byline:afterSave": async (event, ctx) => {
    await ctx.storage.authors.put(event.byline.id, {
      name: event.byline.displayName,
      slug: event.byline.slug,
    });
  },
}
```
...
```ts
interface BylineAfterSaveEvent {
	byline: BylineInfo;
	/** `true` when the byline was just created. */
	isNew: boolean;
}
```

`BylineInfo` has the same public fields that `ctx.bylines.get()` returns. It omits the linked user account, guest flag, and custom field values.
...
Runs after a byline is deleted. The event carries the byline as it was before deletion.

#### Event

```ts
interface BylineAfterDeleteEvent {
	byline: BylineInfo;
}
```
