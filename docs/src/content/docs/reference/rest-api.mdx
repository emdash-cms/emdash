...
`label` and `labelSingular` belong to one locale's definition. `hierarchical` and `collections` belong to the taxonomy: every locale returns the same values, and an update that sends either changes it for every locale. Creating a definition for a name that already exists in another locale adds it to that taxonomy, whether or not the request sends `translationOf`. The new definition takes the taxonomy's `hierarchical` and `collections`, and a create that sends different values returns `VALIDATION_ERROR`.
