...
`POST /_emdash/api/content/{collection}/{id}/terms/{taxonomy}` replaces the entry's assignments immediately by default. Pass `"stage": true` with `termIds` to save a draft when the entry is published and the collection supports revisions. Publish or discard that draft with the content endpoints. For other entries, `stage: true` applies the terms immediately.
