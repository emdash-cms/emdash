...
references, and exact counts. A failed item with `lastErrorCode: "MEDIA_USAGE_RESOURCE_LIMIT"`
contains a changed live or draft projection with more than 14 extracted media occurrences. It
failed before any projection writes and did not consume the normal retry sequence.
...
replaces newer work or exposes its lease token. Retrying `MEDIA_USAGE_RESOURCE_LIMIT` without first
reducing the changed projection to 14 occurrences or fewer, or reconciling it through
operator-controlled collection repair after assessing that collection-wide operation's cost,
returns the job to the same terminal state. Oversized projections that already match the stored
index remain valid no-ops.

These endpoints operate only on durable work already known for an entry. They do not scan for
historical gaps. Use collection-scoped Media Usage repair after imports or direct database writes.
