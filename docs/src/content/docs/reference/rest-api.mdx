...
Administrators can run the same procedure from **Settings → Media Usage** by selecting **Enable Media Usage**, then **Turn on**. The page sends serialized progress requests while it remains visible.
...
4. Call the progress `POST` endpoint serially. Follow `nextRequestInMs` until it returns `null`.
5. Resume direct database writes after activation becomes `active`.
6. Continue progress requests until historical indexing reports `ready`.
...
Failed jobs remain visible and manually retryable. A `needs_attention` progress state stops automatic requests from the Settings page until the underlying failure is resolved.
