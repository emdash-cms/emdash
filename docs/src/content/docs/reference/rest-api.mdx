...
If media usage tracking was not enabled during initial setup, it must be turned on once for the site. Pause direct database writes while EmDash prepares each collection. EmDash temporarily blocks its own content and schema writes during this step.
