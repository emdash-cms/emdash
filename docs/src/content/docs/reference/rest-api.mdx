...
1. Request an upload target
	...
	| Field         | Description                                                       |
	| ------------- | ----------------------------------------------------------------- |
	| `contentHash` | Content hash used to find an existing matching media item          |
	| `fieldId`     | Field whose configured MIME type allowlist applies to the upload   |
	...
	contains `existing: true`, `mediaId`, `storageKey`, and `url` instead of an upload target.
...
| Status | Code                   | Cause                                                               |
| ------ | ---------------------- | ------------------------------------------------------------------- |
| `400`  | `NO_FILE`              | The multipart request has no `file` field                           |
| `400`  | `INVALID_TYPE`         | The MIME type is not allowed                                        |
| `400`  | `VALIDATION_ERROR`     | Upload target metadata is missing, invalid, or exceeds the size limit |
...
| `413`  | `PAYLOAD_TOO_LARGE`    | A direct multipart upload exceeds `maxUploadSize`                    |
