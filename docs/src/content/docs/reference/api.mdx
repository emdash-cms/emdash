...
| `getMenuWithCacheHint` | Rendering a navigation menu with cache invalidation |
...
| `getSiteSettingsWithCacheHint` | Reading global settings with cache invalidation |
| `getTaxonomyTermsWithCacheHint` | Rendering taxonomy filters with cache invalidation |
| `getTermsForEntries` | Batch-loading one taxonomy for a list of entries |
| `sanitizeHref` | Rejecting unsafe URL schemes before rendering stored links |
...
Preview mode is handled automatically: when the request has a valid `_preview` token, the query serves draft content. Preview state requires no parameter.

`references` names the reference fields to load, keyed by field slug. `true` requests the first page at the default limit of 50 entries; the object form takes a `limit` (at most 100) and the `cursor` from a previous page. Fields left out are not read, and a call that omits the option runs no reference queries.
...
`entry.references` holds one page per reference field that `getEmDashEntry` was asked for, keyed by field slug:
...
Each entry is mapped the way a directly loaded entry is, with one exception: bylines and taxonomy terms are not hydrated, so `data.bylines` and `data.terms` are absent.

### `getEmDashReferences()`

Fetch one page of a single reference field, without re-reading the entry it hangs off. Use it to walk past the first page with the `nextCursor` that page returned.
...
The following example loads the next 20 linked entries:
...
`error` is set only for actual errors. An unknown field, a missing entry, or one the request may not see all resolve to an empty `entries` array.

The result also carries a `cacheHint` naming the rows the page read. A route that caches what it renders should merge it into its own hint, the way `getEmDashEntry({ references })` folds in the first page's, so that writing to one of those entries expires the page.
...
These helpers accept `http:`, `https:`, `mailto:`, `tel:`, site-relative paths, and fragments. They reject script-capable schemes, protocol-relative URLs, backslash-prefixed forms that browsers resolve to another site, and control characters. `isSafeHref(value)` returns a boolean. `sanitizeHref(value)` returns the original safe URL or `"#"` when the value is empty or unsafe.
...
`getTaxonomyDefs({ locale? })` lists one definition per taxonomy, and `getTaxonomyDef(name, { locale? })` returns one definition, or `null` when no locale defines the taxonomy. Both resolve a taxonomy in every locale; [Translate taxonomies and terms](/guides/taxonomies/#translate-taxonomies-and-terms) explains which locale's label they use. Term lookups follow the configured fallback chain.
...
With `scope: "title"` the query matches only each collection's title field instead of the full indexed text -- useful for pickers and autocomplete where body-text matches would surprise. Collections whose title field is not indexed for search return no results under this scope.
