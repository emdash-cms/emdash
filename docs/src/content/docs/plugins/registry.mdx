...
| **Moderation** | The operator vets listings and can remove them | Labelers issue takedown and verification labels you choose to accept |
| **Who you trust** | The marketplace operator | The aggregator you point at, plus any labelers you accept |
...
The bare URL string is shorthand. Use the object form when you need a labeler or a release-age policy:
...
- **`acceptLabelers`** — a comma-separated list of labeler DIDs forwarded to the aggregator. Labelers apply takedown and verification labels; the aggregator filters and annotates results according to the ones you accept. When unset, the aggregator applies its operator default set.
...
<Aside type="caution">
	...
	`acceptLabelers` widen the takedown window but assume the labeler system is
	operating.
</Aside>
