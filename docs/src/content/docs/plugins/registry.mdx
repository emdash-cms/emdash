...
- **`acceptLabelers`** — an optional comma-separated list of labeller DIDs forwarded to the aggregator. When unset, the aggregator applies its policy. When set, the list must include every labeller the aggregator requires; a caller cannot disable a required approval or takedown source.
