...
The runner most sites use today is `sandbox()` from `@emdash-cms/cloudflare`, which uses Cloudflare Workers' [Dynamic Worker Loader](https://developers.cloudflare.com/workers/runtime-apis/bindings/worker-loader/) to spawn a fresh isolate per plugin invocation. Runners for other platforms are in development.
...
<CardGrid>
	...
	<LinkCard
		...
		href="/plugins/creating-native-plugins/when-to-go-native/"
