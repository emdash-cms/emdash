...
<Aside type="caution">
	A sandboxed plugin cannot cancel a save: the runner logs a thrown error and the save continues. Cancelling works in a native plugin, which runs in the host process.
</Aside>

To cancel the save from a native plugin, throw `ContentSaveRejectedError` (exported from `emdash`): the API responds with a `SAVE_REJECTED` error that carries your message, and the admin shows it to the editor. Any other thrown error also cancels the save, but the response replaces its message with a generic one.
