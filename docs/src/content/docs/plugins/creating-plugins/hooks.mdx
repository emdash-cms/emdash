...
| `content:beforePublish`, `content:beforeSchedule`, `content:beforeUnpublish` | `hooks.content-policy:register` | The hooks can reject publication state changes. |
...
**Event:** `{ content, collection, isNew, id, actor }` — **Returns:** modified content, a sandbox hook error result, or `void`. On an update, `id` is the ID of the existing item and `content` holds only the submitted field values; load the stored item with `ctx.content.get(event.collection, event.id)`. The entry's slug is not part of `content`, and a `slug` key returned from the hook fails validation as an unknown field. Authenticated REST, visual editing, and MCP saves include `actor.id` and the numeric `actor.role`. Internal writes without an authenticated user omit `actor`.
...
The following hook requires an approval marker before content can become live:
