...
On the Cloudflare sandbox adapter, `putMany()` writes items sequentially. If a write fails, the promise rejects, earlier writes remain committed, and later items are not attempted.
