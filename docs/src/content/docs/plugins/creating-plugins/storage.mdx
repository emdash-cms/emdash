...
<Aside type="note">
	Storage is scoped to the plugin id. A `submissions` collection in the `forms` plugin is completely separate from `submissions` in another plugin — the plugin can only see its own data.
</Aside>
