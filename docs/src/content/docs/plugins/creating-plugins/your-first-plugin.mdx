...
- **`id` is a URL-safe slug, not the npm package name.** Plugin routes mount at `/_emdash/api/plugins/<id>/...` as a single path segment, so `id` must match `^[a-z0-9-]+$` (or the legacy `@scope/name` form for plugins published before the slug-only convention was enforced). Real plugins typically pair an unscoped `id` like `plugin-hello` with a scoped npm package name in `entrypoint`.
