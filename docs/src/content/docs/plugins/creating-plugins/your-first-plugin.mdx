...
`emdash-plugin build` reads both and emits the `dist/` artifacts a site consumes. You do not write a descriptor or a build script.
...
1. Create the directory and a `package.json`. The build is `emdash-plugin build`; there is no `tsdown` invocation to write.
   ...
   `"."` is the auto-generated descriptor a site imports. `"./sandbox"` is the built runtime file. The build produces both, so you never hand-write either.
...
`emdash-plugin.jsonc` carries the plugin's identity (`slug`), its trust contract (`capabilities`, `allowedHosts`, `storage`), profile fields, and the [publisher pin](/plugins/creating-plugins/manifest/#publisher-pinning).
...
`src/plugin.ts` default-exports a bare object annotated with `satisfies SandboxedPlugin`. The import from `emdash/plugin` is type-only, so the `emdash` runtime does not enter the plugin bundle.
...
- [Bundling and publishing](/plugins/creating-plugins/publishing/) — shipping to the marketplace
