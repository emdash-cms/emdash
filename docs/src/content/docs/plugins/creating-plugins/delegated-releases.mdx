...
3. Review and commit the generated workflow.
   ...
   If the repository contains `.changeset/config.json`, interactive setup offers Changesets version updates as the default trigger. That workflow watches the configured base branch and publishes plugin packages whose versions change in a push. Otherwise, the generated workflow runs for package tags matching `<slug>@<version>`. Both variants support `workflow_dispatch` and can be selected explicitly with `--trigger changesets|tags|manual`.
...
5. Start the release workflow.
   ...
   With Changesets, merge the version pull request. A base-branch push that changes the package version starts the release automatically. Changes to dependencies or other package metadata without a version bump do not publish the plugin.
