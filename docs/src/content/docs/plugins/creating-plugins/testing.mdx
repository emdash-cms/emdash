...
- `createPluginRuntimeTestHost()` runs real EmDash content, lifecycle, media, comment, scheduled-task, and plugin-route actions. Use it when the test must prove that a host action reaches the plugin.
...
- `actions` runs content state changes, plugin lifecycle transitions, media uploads, comment submission and moderation, and policy-checked plugin routes.
- `inspect` reads content, plugin storage, KV, settings, plugin state, scheduled tasks, media, comments, and captured email.
- `scheduled` controls the effective scheduler time and runs the production task executor.
...
The runtime host exposes shipped operations only. Capability-specific helpers for translations, publication policy, Block Kit interactions, binary HTTP, raw routes, taxonomies, redirects, expanded comment administration, media bytes, and encrypted settings belong to the releases that add those capabilities.
