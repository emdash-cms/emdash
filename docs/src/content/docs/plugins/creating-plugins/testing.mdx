...
Neither host renders the EmDash admin application or reproduces Cloudflare's deployed CPU or memory limits. The default Worker Loader host does enforce EmDash's bridge-call subrequest budget. Use a disposable EmDash site for browser journeys, and verify CPU- or memory-sensitive behavior on a Cloudflare preview or staging deployment.
