...
- `inspect` reads content, byline credits, taxonomy assignments, plugin storage, KV, settings, plugin state, scheduled tasks, media, comments, and captured email.
...
The runtime host exposes shipped operations only. Capability-specific helpers for publication policy, Block Kit interactions, binary HTTP, raw routes, taxonomy writes, redirects, expanded comment administration, media bytes, and encrypted settings belong to the releases that add those capabilities.
