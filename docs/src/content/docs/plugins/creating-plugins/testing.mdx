...
- `inspect` reads content, plugin storage, KV, settings, plugin state, scheduled tasks, publication-policy rejections, media, comments, and captured email. Use `inspect.scheduledPolicyRejections()` to verify that a scheduler veto was persisted for administrator attention.
...
The runtime host exposes shipped operations only. Capability-specific helpers for translations, Block Kit interactions, binary HTTP, raw routes, taxonomies, redirects, expanded comment administration, media bytes, and encrypted settings belong to the releases that add those capabilities.
