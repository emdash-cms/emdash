...
- `fixtures` creates site, collection, field, user, byline, taxonomy, content, redirect, and plugin state without firing hooks.
...
- `inspect` reads content, redirects, byline credits, taxonomy assignments, plugin storage, KV, settings, plugin state, scheduled tasks, media, comments, and captured email.
...
Use `host.fixtures.redirect()` to establish redirect state without invoking the plugin. Use `host.inspect.redirects()` to assert the persisted rules after the plugin calls `ctx.redirects`. Capability-specific helpers for translations, publication policy, Block Kit interactions, binary HTTP, raw routes, taxonomies, expanded comment administration, media bytes, and encrypted settings belong to the releases that add those capabilities.
