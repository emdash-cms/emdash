...
- `inspect` reads content, redirects, byline credits, taxonomy assignments, plugin storage, KV, raw persisted setting envelopes, plugin state, scheduled tasks, media metadata and bytes, comments, and captured email.
...
The runtime host exposes shipped operations only. Capability-specific helpers for publication policy, Block Kit interactions, binary HTTP, raw routes, and expanded comment administration belong to the releases that add those capabilities.
