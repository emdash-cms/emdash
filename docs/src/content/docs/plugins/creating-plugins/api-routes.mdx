...
<Aside type="caution">
	Plugin ids must be URL-safe slugs — `[a-z0-9-]+`. The `pluginId` segment in the route is a single Astro path segment, so an id containing `/` (such as a scoped npm package name) won't route. Pair an unscoped id with a scoped npm package name in `entrypoint`, the way first-party plugins do (`id: "field-kit"`, `entrypoint: "@emdash-cms/plugin-field-kit"`).
</Aside>
