---
title: Block Kit
description: Declarative UI blocks for plugin pages, widgets, and saved-entry extensions.
---
...
<Aside>
	Native plugins can ship their own React components for admin pages. Block Kit is the admin UI system for sandboxed plugins, where shipping React would break sandbox isolation. Native plugins can use Block Kit too: see [Block Kit in native plugins](#block-kit-in-native-plugins).
</Aside>
...
EmDash validates every sandboxed page and widget response before the admin renders it. An invalid block, unsafe URL, link to an undeclared plugin page, or response over the Block Kit limits fails the request instead of reaching the browser. A response can contain up to 256 KiB, 20 nested levels, 2,000 nodes, 1,000 items per array, and 64 KiB per string.
...
Use `refresh: true` to reload the entry, `navigate` with a structured link target, or `patch` to propose unsaved field changes. A response cannot combine terminal effects. EmDash rejects unknown commands, unsafe navigation, invalid or stale patches, and responses over the Block Kit limits before applying an effect.

## Block Kit in native plugins

A native plugin can render Block Kit pages and widgets without shipping React. Declare `admin.pages` or `admin.widgets` in `definePlugin()`, leave `admin.entry` unset, and add a route named `admin`:
...
A native plugin uses Block Kit when it declares pages, widgets, or editor extensions and no `admin.entry`. Setting `admin.entry` switches it to [React admin pages](/plugins/creating-native-plugins/react-admin/). Declare the `admin` route explicitly: the implicit `admin` route exists only for older sandboxed bundles. A native handler receives a single `RouteContext`, so the interaction arrives as `ctx.input`.

Two behaviors described above currently apply only to sandboxed plugins on admin pages and dashboard widgets:

- `ctx.ui` is `undefined` on a native plugin's page or widget, so the route cannot read the admin locale or text direction from it. Native editor panels and actions do receive `ctx.ui`.
- EmDash does not validate a native plugin's page or widget response before the admin renders it. Keep native responses within the same block types, limits, and link and image rules, because the same renderer draws them.
...
| `tab`       | Labelled panels containing nested blocks                                                 |
...
Use the [Block Playground](https://blocks.emdashcms.com/) to interactively build and test block layouts.
