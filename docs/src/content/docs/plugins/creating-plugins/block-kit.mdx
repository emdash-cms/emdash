...
Set a table column's `format` to `element` to place a button, link, or menu in each row. Each row stores the element under the column's key; a row without a value leaves the cell empty. Use a `menu` element when a row offers several choices behind one button:
...
Choosing a menu item sends a `block_action` with the menu's `action_id` and the item's `value`. Item values must be unique within a menu. Element cells accept only `button`, `link`, and `menu` elements. A menu can also appear in an `actions` block, as a section accessory, or in empty-state actions, but not as a form field. The `elements.menu(actionId, label, items, { style })` builder returns the same shape.
...
| `menu`          | Button that opens a list of choices; each choice dispatches an action |
