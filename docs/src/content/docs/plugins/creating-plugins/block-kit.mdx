...
EmDash validates every page and widget response before the admin renders it. An invalid block, unsafe URL, or link to an undeclared plugin page fails the request instead of reaching the browser.
...
`routeCtx.ui` contains the surface (`admin-page` or `dashboard-widget`), locale, and text direction. The admin locale is separate from `ctx.site.locale`, which describes the site's default content locale. Manifest page labels remain static strings.
