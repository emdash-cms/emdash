...
4. Tarballs. Inside the tarball, `plugin.mjs` is packed as `backend.js` to match the registry's wire-side filename. The output is `dist/<slug>-<version>.tar.gz`.
