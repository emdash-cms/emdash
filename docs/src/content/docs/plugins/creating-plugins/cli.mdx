...
| `--trigger <mode>` | `auto` | Release trigger: `changesets`, `tags`, or `manual`. `auto` selects Changesets when `.changeset/config.json` exists. |
| `--force` | `false` | Replace an existing generated workflow. Without it, setup leaves the existing file unchanged. |

When setup detects Changesets in an interactive terminal, it asks whether Changesets version updates, package tags, or manual runs should start releases. In non-interactive use, `auto` selects Changesets when a valid root configuration exists and package tags otherwise. Pass `--trigger` to make the choice explicit.

The Changesets workflow watches the configured `baseBranch` and plans releases from plugin package version changes in each push. It ignores package metadata changes that keep the same version. Private plugins require `privatePackages.version: true` in `.changeset/config.json`; setup prints a warning when the current plugin would be skipped.
...
`release plan` is used by the generated Changesets workflow. With `--since <git-revision>`, it compares plugin package versions with that revision and writes a JSON selector matrix to `GITHUB_OUTPUT`. With `--package <slug[@version]>`, it validates one manual selector. The command does not build or publish packages.
