...
| `bylines:read`                      | `ctx.bylines.get()`, `ctx.bylines.list()`, `ctx.bylines.getEntriesBylines()`, and the `byline:afterSave` and `byline:afterDelete` hooks |
...
- **Bylines are separate from content and users.** `bylines:read` does not grant `content:read` or `users:read`, and neither of those grants byline access. Declare `content:read` as well if the plugin lists entries to look up their credits.
...
`bylines:read` returns public byline profiles: slug, display name, bio, website URL, avatar media ID, locale, and translation group. It omits the linked user account, the guest flag, and byline custom field values. Credit lookups return the bylines a site renders for an entry: the credits assigned in the editor, or the byline linked to the entry's author when the entry has none. Each credit's `source` is `explicit` or `inferred` accordingly. Credits resolve at the entry's own locale. Trashed and missing entries return no credits.

Use `getEntriesBylines()` to resolve credits for a page of entries in one call. It accepts up to 100 entry IDs from one collection and returns results in the order given:
...
for (const { entryId, bylines } of credits) {
	const names = bylines.map((credit) => credit.byline.displayName);
	ctx.log.info("Entry credits", { entryId, names });
}
```
...
`ctx.media.upload()` accepts image, video, audio, and PDF content and throws for any other content type. In a trusted plugin, `upload()` and `getUploadUrl()` enforce the default media upload allowlist: PNG, JPEG, GIF, WebP, and AVIF images, any `video/*` or `audio/*` type, and `application/pdf`. Other types throw a `PluginRouteError` with status `415`, and a malformed content type throws one with status `400`; a route handler can let either propagate as the response. In a trusted plugin, a file stored by `upload()` or reserved by `getUploadUrl()` also takes an extension that matches the content type, whatever extension the filename has; when the content type has no known extension, the filename's extension is kept only if it belongs to an allowed media type. A sandboxed plugin keeps the filename's extension when it is 1 to 10 letters or digits.
