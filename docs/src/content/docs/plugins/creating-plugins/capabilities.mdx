...
5. **Resource limits.** The Cloudflare runner defaults to 50 ms of CPU, 30 subrequests, and 30 seconds of wall time per invocation. EmDash and Worker Loader enforce subrequests; Worker Loader enforces CPU; the runner enforces wall time. Each bridge-backed `ctx` operation spends one subrequest, so use batch storage methods when an invocation handles several records. Worker Loader has a platform memory ceiling, but its per-plugin `memoryMb` option is not currently enforceable. The site operator can override the defaults with `sandboxLimits`. The Node.js workerd runner enforces only the wall-time setting; it warns when a site configures CPU, memory, or subrequest limits that standalone workerd cannot enforce. A per-hook `timeout` applies only when the sandboxed-format plugin runs in process.
