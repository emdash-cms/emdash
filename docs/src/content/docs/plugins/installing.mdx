...
1. **Sandbox runner configured** — Marketplace plugins run in an isolated runtime, so the runner setup depends on your deployment platform.
   ...
   The Cloudflare adapter reads this config at build time. If the binding is absent, it disables the sandbox runner and logs a single explanatory line, allowing the same project to deploy on the Workers free plan.
