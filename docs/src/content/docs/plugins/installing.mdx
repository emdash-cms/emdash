...
1. **Sandbox runner configured** — Marketplace plugins run in an isolated runtime, so the runner setup depends on your deployment platform.

   On **Cloudflare Workers**, use the Cloudflare sandbox adapter:
   ...
   Sandboxed plugins use Worker Loader, which is available on Workers paid plans. Add the binding to `wrangler.jsonc` to enable them:

   ```jsonc title="wrangler.jsonc"
   "worker_loaders": [{ "binding": "LOADER" }],
   ```

   The Cloudflare adapter reads this config at build time. Without the binding, it disables the runner so the site can deploy on the Workers free plan. The marketplace and registry remain available to browse, but sandboxed plugins under `sandboxed: []` do not load and install requests fail with `SANDBOX_NOT_AVAILABLE`.

   On **Node.js**, install the workerd sandbox runner:
   ...
   The runner includes Miniflare as an optional dependency. With optional dependencies enabled, development uses it automatically for faster sandbox startup.
