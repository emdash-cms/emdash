...
<Aside type="caution">
	...
	example, `redirects:write` allows the plugin to change where visitors are sent.
</Aside>
...
Updates move forward only. To return to an older release, uninstall the plugin and install that version from its registry page. Uninstalling keeps the plugin's stored data unless you choose to delete it.
