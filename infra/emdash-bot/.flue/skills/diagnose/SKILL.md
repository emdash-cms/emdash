...
6. **Rate confidence in the root cause.** This axis is *only* how sure you are you found the responsible code -- not how easy the fix is.
   ...
   - **Low** -- multiple indistinguishable causes, or the right area but no specific defect visible.
     Rate honestly both ways. Fix does not run at `low` but *does* run at `medium` when the fix is clear -- do not reflexively rate down. A confidently located cause is `high` even when the fix involves choosing between options; that choice is the next field's job.
