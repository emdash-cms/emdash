...
**The fix loop does not open a PR.** Fix produces a verified change on a candidate branch, the workflow pushes it and posts a **preview build** to the issue, and the reporter is asked to confirm it resolves _their_ case. **Only after the reporter confirms** does a draft PR open (carrying the repro test, referencing the issue). Reporter denial or silence reaps the branch. Nothing you do here lands on `main`; a maintainer reviews the eventual PR.
