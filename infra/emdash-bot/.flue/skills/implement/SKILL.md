...
5. Use `run_check` for every final verification. At minimum run the focused test and the repository-prescribed lint/typecheck commands that apply. Give checks stable names when rerunning them; publication requires every latest named result to pass.
...
- No output pipelines or `|| true` on final checks. `run_check` rejects status-masking commands.
