...
- A conventional-commit message the workflow can use: `fix(<scope>): <short description> (#<issue>)`, scope matching the package or area (`fix(core/menus)`, `fix(admin/seo)`, `fix(migrations)`).
...
The workflow reads this, pushes the candidate branch, triggers the preview build, and posts the outcome. It does not open a PR until the reporter confirms the preview fixes their case.
