...
Generated collection types include each retained block version, and `defineBlockComponents<T>()` checks that a component map covers every generated `_type`. Image, file, repeater-image, and Portable Text media inside blocks participate in normalization, MIME validation, usage tracking, and cleanup protection.
