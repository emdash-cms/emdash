...
Also fixes the plugin's delivery counts always being 0. Each content or media webhook now records its outcome, HTTP status and duration in the plugin's `deliveries` storage collection, which keeps the 500 most recent deliveries; the `status` route and the dashboard widget's Delivered and Failed counts read from it. The Test Webhook button's sends are not recorded.
