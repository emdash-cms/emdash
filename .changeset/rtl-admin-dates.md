...
Dates were formatted with `toLocaleDateString()` and no locale, which follows the *browser's* language rather than the one chosen in the admin, so a Hebrew admin in an English browser printed English months. Every call site now passes the admin's locale through a shared `formatDate`.
