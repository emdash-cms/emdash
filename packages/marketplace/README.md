...
```bash
curl -s https://marketplace.emdashcms.com/api/v1/auth/discovery | jq '.github.clientId'
```

The response must be a non-empty string. If it is empty, interactive `emdash plugin publish` login will fail before a device code is shown.
