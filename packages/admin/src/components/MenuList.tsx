...
export function MenuList() {
	...
	const { data: menus, isLoading } = useCachedQuery({
		...
		cache: { store: "singletons", key: `menus:${activeLocale ?? "en"}` },
	});
	...
	const deleteMutation = useMutation({
		...
		async onMutate(name: string) {
			await queryClient.cancelQueries({ queryKey: ["menus", activeLocale] });
			const previous = queryClient.getQueryData<unknown[]>(["menus", activeLocale]);
			if (previous) {
		...
		onError: (_error, _name, context) => {
			if (context?.previous) {
				queryClient.setQueryData(["menus", activeLocale], context.previous);
			}
		},
		onSettled: () => {
			void queryClient.invalidateQueries({ queryKey: ["menus", activeLocale] });
		},
	});
