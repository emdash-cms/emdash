...
describe("ContentEditPage – publish button stays in sync after publishing (#1557)", () => {
	...
	it("flips 'Publish updates' to 'Unpublish Post' after a successful publish", async () => {
		...
		const publishBtn = screen.getByRole("button", { name: "Publish updates", exact: true });
		...
		await screen.getByRole("menuitem", { name: /Publish updates now/ }).click();
...
describe("ContentEditPage – publish button appears after saving an edit (#1557)", () => {
	...
	it("shows 'Publish updates' after editing the title and saving", async () => {
		...
		await expect
			.element(screen.getByRole("button", { name: "Publish updates", exact: true }))
			.toBeInTheDocument();
	});
});
...
describe("ContentEditPage – publish button appears after an autosave", () => {
	...
	it("shows 'Publish updates' after editing the title and letting autosave fire", async () => {
		const { router, TestApp } = buildRouter();
