...
describe("ContentEditPage – publish button stays in sync after publishing (#1557)", () => {
	...
	it("flips 'Publish changes' to 'Unpublish Post' after a successful publish", async () => {
		...
		const publishBtn = screen.getByRole("button", { name: "Publish changes", exact: true });
...
describe("ContentEditPage – publish button appears after saving an edit (#1557)", () => {
	...
	it("shows 'Publish changes' after editing the title and saving", async () => {
		...
		// Loads in the clean published state: "Unpublish Post" present, no "Publish changes".
		await expect
			.element(screen.getByRole("button", { name: "Unpublish Post" }))
			.toBeInTheDocument();
...
describe("ContentEditPage – publish button appears after an autosave", () => {
	...
	it("shows 'Publish changes' after editing the title and letting autosave fire", async () => {
		const { router, TestApp } = buildRouter();
...
describe("ContentEditPage – tag-only draft", () => {
	let mockFetch: ReturnType<typeof createMockFetch>;
	...
	beforeEach(() => {
		...
		mockFetch
			...
			.on("GET", "/_emdash/api/content/posts/post_1", {
				data: { item: publishedClean() },
			})
			.on("GET", "/_emdash/api/taxonomies", {
				data: {
					taxonomies: [
						{
							id: "tags",
			...
			.on(
				"GET",
				"/_emdash/api/taxonomies/tags/terms?includeCounts=false&resolveFallback=true&locale=en",
				{
					data: {
						terms: [
							{
			...
			.on("GET", "/_emdash/api/content/posts/post_1/terms/tags", {
				data: { terms: [], unresolved: [], entryLocale: "en", defaultLocale: "en" },
			})
			.on("GET", "/_emdash/api/revisions/rev_1", {
				data: {
					item: {
			...
			.on("POST", "/_emdash/api/content/posts/post_1/terms/tags", {
				data: { terms: [], staged: true, _rev: "new-revision" },
			});
	});

	afterEach(() => mockFetch.restore());

	it("offers Publish changes after selecting a tag without touching the content field", async () => {
		const { router, TestApp } = buildRouter();
		await router.navigate({
			to: "/content/$collection/$id",
			params: { collection: "posts", id: "post_1" },
		...
		await expect
			.element(screen.getByRole("button", { name: "Unpublish Post" }))
			.toBeInTheDocument();
		await screen.getByRole("button", { name: "Choose Tags" }).click();
		mockFetch.on("GET", "/_emdash/api/content/posts/post_1", {
			data: { item: publishedDirty() },
		});
		await screen.getByRole("checkbox", { name: "Internship Experience" }).click();
		await expect
			.element(screen.getByRole("button", { name: "Publish changes", exact: true }))
			.toBeInTheDocument();
	});

	it("removes the staged tag from the picker when draft changes are discarded", async () => {
		const { router, TestApp } = buildRouter();
		await router.navigate({
			to: "/content/$collection/$id",
			params: { collection: "posts", id: "post_1" },
		...
		mockFetch.on("GET", "/_emdash/api/content/posts/post_1/terms/tags", {
			data: { terms: [selectedTerm], unresolved: [], entryLocale: "en", defaultLocale: "en" },
		});
		await screen.getByRole("checkbox", { name: "Internship Experience" }).click();
		await expect
			.element(screen.getByRole("button", { name: "Publish changes", exact: true }))
			.toBeInTheDocument();
		await expect.element(screen.getByLabelText("Remove Internship Experience")).toBeInTheDocument();
		...
		mockFetch.on("POST", "/_emdash/api/content/posts/post_1/discard-draft", {
			data: { item: publishedClean() },
		});
		...
		await expect
			...
			.toBeInTheDocument();
		await expect
			.element(screen.getByLabelText("Remove Internship Experience"))
