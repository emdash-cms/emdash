...
let revisionRestoredCallback: ((item: ContentItem) => void) | undefined;
vi.mock("../../src/components/RevisionHistory", () => ({
	RevisionHistory: ({ onRestored }: { onRestored?: (item: ContentItem) => void }) => {
		revisionRestoredCallback = onRestored;
		return <div data-testid="revision-history">Revision History</div>;
	},
}));
...
describe("ContentEditor", () => {
	beforeEach(() => {
		...
		revisionRestoredCallback = undefined;
	});
	...
	describe("saving", () => {
		...
		it("cancels a scheduled autosave while discarding the draft", async () => {
			vi.useFakeTimers();
			...
			try {
				...
				await screen.getByLabelText("Title").fill("Edit that should be discarded");
				await screen.getByRole("button", { name: "Discard changes" }).first().click();
				screen.getByRole("button", { name: "Discard changes" }).last().element().click();
		...
		it("preserves edits made while an autosave is in flight", async () => {
			vi.useFakeTimers();
			...
			try {
				...
				await screen.rerender(<ContentEditor {...props} isAutosaving={true} />);
				await titleInput.fill("Newer edit");
				...
				await screen.rerender(
					<ContentEditor
						...
						item={makeItem({
							updatedAt: "2026-04-12T18:38:00Z",
							data: { title: "First edit", body: "Some content" },
						})}
						isAutosaving={false}
						autosaveCompletionToken={1}
					/>,
				...
				await expect.element(screen.getByLabelText("Title")).toHaveValue("Newer edit");
				await vi.advanceTimersByTimeAsync(2000);
				expect(onAutosave).toHaveBeenCalledTimes(2);
				expect(onAutosave).toHaveBeenLastCalledWith(
					expect.objectContaining({
						data: expect.objectContaining({ title: "Newer edit" }),
					}),
				);
			} finally {
				vi.useRealTimers();
		...
		it("preserves newer edits when autosave completion arrives before the cache update", async () => {
			vi.useFakeTimers();
			...
			try {
				...
				await screen.rerender(
					<ContentEditor {...props} isAutosaving={false} autosaveCompletionToken={1} />,
				);
				await screen.rerender(
					<ContentEditor
						{...props}
		...
		it("preserves unsaved edits when a failed autosave is followed by an item update", async () => {
			vi.useFakeTimers();
			...
			try {
				...
				const screen = await render(<ContentEditor {...props} />);
				await screen.getByLabelText("Title").fill("Unsaved edit");
				await vi.advanceTimersByTimeAsync(2000);
				await screen.rerender(<ContentEditor {...props} isAutosaving={true} />);
				...
				await screen.rerender(
					<ContentEditor
						{...props}
						item={makeItem({ updatedAt: "2026-04-12T18:38:00Z" })}
						isAutosaving={false}
						autosaveCompletionToken={0}
					/>,
				);

				await expect.element(screen.getByLabelText("Title")).toHaveValue("Unsaved edit");
			} finally {
				vi.useRealTimers();
			}
		});

		it("preserves newer edits across split autosave cache updates", async () => {
			vi.useFakeTimers();
			...
			try {
				...
				await screen.rerender(
					<ContentEditor
						{...props}
						item={makeItem({
							updatedAt: "2026-04-12T18:38:01Z",
							data: { title: "First edit", body: "Some content" },
							bylines: [{ byline: makeByline(), sortOrder: 0, roleLabel: "Author" }],
						})}
						isAutosaving={false}
						autosaveCompletionToken={1}
					/>,
		...
		it("preserves edits made while a manual save is in flight", async () => {
			...
			await screen.rerender(<ContentEditor {...props} isSaving={true} />);
			await titleInput.fill("Newer edit");
	...
	describe("publish actions", () => {
		...
		it("disables publishing while an editor write is active", async () => {
			const screen = await renderEditor({
				isNew: false,
	...
	describe("translation / item switch", () => {
		...
		it("ignores a stale revision restore after switching entries", async () => {
			const itemA = makeItem({ id: "item-a", data: { title: "Item A", body: "" } });
			const itemB = makeItem({ id: "item-b", data: { title: "Item B", body: "" } });
			const props: ContentEditorProps = {
				collection: "posts",
				collectionLabel: "Post",
				fields: defaultFields,
			...
			const screen = await render(<ContentEditor {...props} />);
			const staleRestore = revisionRestoredCallback;

			await screen.rerender(<ContentEditor {...props} item={itemB} />);
			await screen.getByLabelText("Title").fill("Unsaved B edit");
			await React.act(async () => {
				staleRestore?.(makeItem({ id: "item-a", data: { title: "Restored A", body: "" } }));
			});
			await screen.rerender(
				<ContentEditor
					{...props}
					item={makeItem({
						id: "item-b",
						updatedAt: "2026-04-12T18:38:00Z",
						data: { title: "Item B", body: "" },
					})}
				/>,
			);

			await expect.element(screen.getByLabelText("Title")).toHaveValue("Unsaved B edit");
		});

		it("does not reapply a historical content reset after returning to an entry", async () => {
			const resetItem = makeItem({ id: "item-a", data: { title: "Reset A", body: "" } });
			const itemB = makeItem({ id: "item-b", data: { title: "Item B", body: "" } });
			const props: ContentEditorProps = {
				collection: "posts",
				collectionLabel: "Post",
				...
				item: resetItem,
				onSave: vi.fn(),
				contentResetCompletionToken: 0,
				contentResetItem: resetItem,
			};
			const screen = await render(<ContentEditor {...props} />);

			await screen.rerender(<ContentEditor {...props} contentResetCompletionToken={1} />);
			await screen.rerender(
				<ContentEditor
					{...props}
					item={itemB}
					contentResetCompletionToken={0}
					contentResetItem={undefined}
				/>,
			);
			await screen.rerender(<ContentEditor {...props} contentResetCompletionToken={1} />);
			await screen.getByLabelText("Title").fill("New A edit");
			await screen.rerender(
				<ContentEditor
					{...props}
					item={makeItem({
						id: "item-a",
						updatedAt: "2026-04-12T18:38:00Z",
						data: { title: "Reset A", body: "" },
					})}
					contentResetCompletionToken={1}
				/>,
			);

			await expect.element(screen.getByLabelText("Title")).toHaveValue("New A edit");
		});
	});
