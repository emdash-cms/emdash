 ...
 *
 * Falls back to the source locale directories when the compiled `locales/`
 * directory is missing. This lets a freshly-cloned workspace validate the
 * option before the admin package has been built, without changing behavior
 * for real installs where the compiled catalogs are present.
 */
export function getAdminLocaleCodes(adminDistPath: string): Set<string> {
	const codes = readLocaleCodesFromDir(resolve(adminDistPath, "locales"), "messages.mjs");
	if (codes.size > 0) return codes;

	// Fallback: admin source directory is at `<adminDistPath>/../src`.
	return readLocaleCodesFromDir(resolve(adminDistPath, "..", "src", "locales"), "messages.po");
}

/**
 * Scan a directory for locale subdirectories containing a named catalog file.
 */
function readLocaleCodesFromDir(localesDir: string, catalogFile: string): Set<string> {
	const codes = new Set<string>();

	try {
		for (const entry of readdirSync(localesDir)) {
			const entryPath = resolve(localesDir, entry);
			if (statSync(entryPath).isDirectory() && existsSync(resolve(entryPath, catalogFile))) {
				codes.add(entry);
			}
		}
	} catch {
		// Directory missing or unreadable -- return empty set so callers can
		// decide whether to fall back or fail validation.
	}
...
export function validateAdminLocales(
	...
	if (known.size === 0) {
		throw new Error(
			"Cannot validate `admin.locales`: no admin locale catalogs were found. " +
				"Make sure `@emdash-cms/admin` is installed and built.",
		);
	}
...
export function createAdminLocaleResolverPlugin(options: AdminLocaleResolverOptions): Plugin {
	...
	// The chunk map is only needed for hashed production imports. In dev source
	// mode the source paths are resolved directly, so a missing map is not a
	// problem there; warn only when we are relying on the dist bundle for the
	// allowlist to work.
	if (hashToLocale.size === 0 && !adminSourcePath) {
		console.warn(
			`[emdash-admin-locales] Could not map hashed admin locale chunks in ${adminDistPath}. ` +
				"The allowlist may not reduce the bundle. Make sure `@emdash-cms/admin` is installed and built.",
		);
	}
