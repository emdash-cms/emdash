...
/**
 * Resolve the session user without risking an isolate-wide hang.
 *
 ...
 * directly hangs the request until the platform kills it, and because the
 * stalled promise is shared at the isolate level, every later session-bearing
 * request hangs too (observed as 0-CPU, multi-minute, `canceled` responses;
 * see #1274). A surrounding try/catch cannot help: the promise never rejects.
 *
 ...
 *  2. A timeout is a fail-closed backstop: a still-stalled read resolves to
 *     `undefined`, and every caller treats the absence of a session user as
 *     unauthenticated (anonymous on public routes, 401/redirect on protected
 *     ones). It can only ever drop privileges for that one request, never grant
 *     them.
 */
async function resolveSessionUser<T>(
