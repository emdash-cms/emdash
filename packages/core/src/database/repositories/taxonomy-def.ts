 ...
 *
 * Creates the group with `groupId` when the name has none, and links every
 * definition row of the name to the group, including rows written without it.
 */
export async function saveTaxonomyStructure(
	...
	const hierarchical = structure.hierarchical ? 1 : 0;
	const collections = JSON.stringify([...new Set(structure.collections)]);
	const group = await db
		.insertInto("_emdash_taxonomy_def_groups")
		.values({ id: groupId, name, hierarchical, collections })
		.onConflict((oc) => oc.column("name").doUpdateSet({ hierarchical, collections }))
		.returning("id")
		.executeTakeFirstOrThrow();
	await db
		.updateTable("_emdash_taxonomy_defs")
		.set({ hierarchical, collections, translation_group: group.id })
		.where("name", "=", name)
		.execute();
	return group.id;
}
