...
export async function up(db: Kysely<unknown>): Promise<void> {
	...
	for (const collection of collections.rows) {
		...
		try {
			...
			console.warn(
				`[migration 056] skipping FTS rebuild for collection "${collection.slug}": ${
...
async function getSearchableFields(
	...
	for (const row of rows.rows) {
		try {
			...
			console.warn(
				`[migration 056] skipping invalid searchable field "${row.slug}" on collection "${collectionSlug}"`,
			);
		}
	}
	return out;
