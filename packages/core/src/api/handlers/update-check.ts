 ...
 *
 * Reads the cached latest published `emdash` version from the options
 * table, compares it against the running `VERSION`, and schedules a
 * deferred registry refresh when the cache is stale or missing.
 */
 ...
 * On an equal base version, a stable release is newer than a prerelease
 * of it, so a site running `0.24.0-beta.1` is notified when `0.24.0`
 * ships. Prerelease identifiers are not compared to each other; fine
 * for npm `latest`, which never points at a prerelease.
 */
 ...
 *
 * Concurrent stale reads can kick overlapping refreshes; both
