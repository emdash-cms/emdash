...
/**
 * POST /_emdash/api/auth/totp/login request body.
 *
 * Discriminated union: the client tells the server which method it's
 * using. For the TOTP path, `code` is exactly 6 digits; for the
 * recovery path, `recoveryCode` matches the XXXX-XXXX base32 format
 * generated by generateRecoveryCodes. The format regexes let the zod
 * layer reject malformed input before any HMAC or DB work runs.
 */
export const totpLoginBody = z
	.discriminatedUnion("method", [
		...
		z.object({
			...
			recoveryCode: z
				.string()
				.regex(/^[A-Z2-7]{4}-[A-Z2-7]{4}$/, "Recovery code must look like ABCD-2345"),
		}),
	])
	.meta({ id: "TotpLoginBody" });
