...
describe("FTS write amplification", () => {
	...
	it("does not re-tokenize when the publish path rewrites identical data values", async () => {
		const before = await indexSegments();

		// The publish path SETs every data column even when values are
		// unchanged; only value comparison suppresses those re-tokenizations.
		const row = await sql<{ content: string; title: string }>`
			SELECT content, title FROM ec_pages WHERE id = ${entryId}
		`.execute(db);
		await sql`
			UPDATE ec_pages
			SET content = ${row.rows[0]!.content}, title = ${row.rows[0]!.title}, status = 'published'
			WHERE id = ${entryId}
		`.execute(db);
