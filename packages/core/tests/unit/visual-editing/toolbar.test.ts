...
describe("renderToolbar", () => {
	...
	// Note on #878 (edit-mode toggle reload):
	// The toggle handler is JavaScript executed in the browser; its behaviour
	// (cookie write -> reload -> server re-renders the toolbar with the new
	// edit-mode state) cannot be meaningfully verified by string-matching the
	// rendered HTML in a unit test. The end-to-end coverage lives in
	// `e2e/tests/edit-toggle.spec.ts`, which drives the toggle in a real
	// browser and asserts on the post-reload server-rendered state.
});
