/**
 * Migration 056 rebuilds FTS indexes as self-contained tables indexing
...
describe("migration 056: FTS indexes extracted Portable Text prose", () => {
	...
	/**
	 * Rebuild the pages FTS table in the pre-056 shape: external-content
	...
	async function runMigration056(): Promise<void> {
		const { up } = await import("../../../../src/database/migrations/056_fts_plain_text.js");
	...
	it("is a no-op on databases with no search-enabled collections", async () => {
		...
		await expect(runMigration056()).resolves.toBeUndefined();
	});
});
