...
describe("016_api_tokens migration", () => {
	...
	describe("re-run safety after a partially-applied migration (#954)", () => {
		...
		it("recovers when only the first table was created before the crash", async () => {
			// Simulate the partial-apply state directly: the first
			// `createTable("_emdash_api_tokens")` succeeded, then `up()`
			// crashed before creating the OAuth or device-code tables.
			await db.schema
