...
describe("query helpers fallback locale resolution", () => {
	...
	it.each([
		["with i18n", { defaultLocale: "id", locales: ["id", "en"] }, false],
		["without i18n", null, false],
		["in edit mode", { defaultLocale: "id", locales: ["id", "en"] }, true],
	])(
		"getEmDashEntry returns no entry and no error when the entry is missing %s",
		async (_, i18n, editMode) => {
			setI18nConfig(i18n);
			vi.mocked(getLiveEntry).mockImplementation(
				async () =>
					({ entry: undefined, error: new LiveEntryNotFoundError("post", "missing") }) as any,
			);

			const { entry, error, isPreview } = await runWithContext({ editMode, db }, () =>
				getEmDashEntry("post", "missing", { locale: "en" }),
			);

			expect(error).toBeUndefined();
			expect(entry).toBeNull();
			expect(isPreview).toBe(editMode);
		},
	);
});
