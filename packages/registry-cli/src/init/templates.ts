/**
 * Pure file-content producers for `emdash-registry init`.
 *
 ...
 * `emdash-registry bundle` transpiles at publish time.
 */
...
export function renderManifest(input: ScaffoldInputs): string {
	...
	if (!input.publisher) {
		lines.push(
			'\t// TODO: set your atproto handle (e.g. "example.com") or DID before running `emdash-registry bundle` or any local-dev integration. The plugin cannot load without it.',
		);
...
To test against a running EmDash site, link this plugin into the site's
workspace and reference it from the integration's plugin loader. (Local-
dev wiring details land alongside the \`localPlugin\` helper from
\`@emdash-cms/registry-cli/dev\`.)

## Publish

\`\`\`sh
emdash-registry login        # if you're not already logged in
emdash-registry bundle       # produces dist/${title}-<version>.tar.gz
# upload that tarball to a public URL, then:
emdash-registry publish --url https://your-host/...
\`\`\`
