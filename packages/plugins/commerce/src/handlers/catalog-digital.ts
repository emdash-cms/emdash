...
export async function handleCreateDigitalAsset(
...
export async function handleCreateDigitalEntitlement(
	...
	if (sku.status !== "active") {
		throw PluginRouteError.badRequest(
			`Cannot attach entitlement to inactive SKU ${ctx.input.skuId}`,
		);
	}
...
export async function handleRemoveDigitalEntitlement(
	...
	if (!existing) {
		throwCommerceApiError({
			code: "DIGITAL_ENTITLEMENT_NOT_FOUND",
			message: "Digital entitlement not found",
		});
	}
