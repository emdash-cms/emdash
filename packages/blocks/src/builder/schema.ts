      ...
      if (!Array.isArray(cols)) {
        ...
        cols.forEach((col: unknown, i: number) => {
          ...
          if (!Array.isArray(colBlocks)) {
            errors.push({ path: `${path}.columns[${i}].blocks`, message: 'Field blocks must be an array' });
          } else {
...
export function validateBuilderDocument(
  ...
  if (!Array.isArray(doc.blocks)) {
    return { valid: false, errors: [...errors, { path: 'blocks', message: 'Field blocks must be an array' }] };
  }
