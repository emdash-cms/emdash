...
function lexicalTextToPortableSpan(node: SerializedLexicalNode) {
  return {
    ...
    ...(node.underline === true || hasTextFormat(node, TEXT_FORMAT_UNDERLINE) ? { underline: true } : {}),
    ...(node.strikethrough === true || hasTextFormat(node, TEXT_FORMAT_STRIKETHROUGH) ? { strikethrough: true } : {}),
  ...
  switch (node.type) {
    ...
    case 'paragraph':
      ...
      if (node.children && node.children.length > 0) {
        ...
        const level = isHeading && typeof node.tag === 'string'
