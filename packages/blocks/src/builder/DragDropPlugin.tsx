...
export function DragDropPlugin({
    ...
    (event: DragEndEvent) => {
      ...
      if (overId && nodeKeysArray.includes(overId)) {
        ...
        const clientY = event.activatorEvent instanceof MouseEvent
          ...
          : (event.over?.rect.top ?? rootRect.top) + (event.over?.rect.height ?? rootRect.height) / 2;
  ...
  const overlayContent = dragState ? (
    <div className="lexical-drag-overlay">
      <span className="lexical-drag-overlay__label">
        {dragState.activeNodeType}
      </span>
    </div>
  ) : null;
