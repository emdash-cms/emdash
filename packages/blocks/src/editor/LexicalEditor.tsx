...
function EditorContextBridge({ children }: { children: React.ReactNode }) {
	...
	return <LexicalEditorContext.Provider value={editor}>{children}</LexicalEditorContext.Provider>;
}
	...
	const editorElement = (
		<DragDropPlugin>
			<div className={`lexical-editor ${className}`}>
				...
				/>
				<HistoryPlugin />
				{onChange && <OnChangePlugin onChange={handleChange} ignoreSelectionChange={false} />}
			</div>
		</DragDropPlugin>
	);
