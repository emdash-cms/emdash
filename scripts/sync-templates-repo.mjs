...
for (const template of TEMPLATES) {
	...
	if (existsSync(srcPkg)) {
		writeFileSync(join(destDir, "package.json"), transformPackageJson(srcPkg, catalog, workspace));
...
const diff = run("git diff --stat", { cwd: targetDir });
const untracked = run("git ls-files --others --exclude-standard", { cwd: targetDir });
if (!diff && !untracked) {
...
if (process.env.CI) {
	...
	run('git config user.email "github-actions[bot]@users.noreply.github.com"', { cwd: targetDir });
}

run(`git checkout -b ${branch}`, { cwd: targetDir });
run("git add -A", { cwd: targetDir });
run(`git commit -m "chore: sync templates from emdash v${emdashVersion}"`, { cwd: targetDir });
run(`git push -u origin ${branch}`, { cwd: targetDir });
...
const prUrl = run(
	`gh pr create --repo ${REPO} --title "chore: sync templates from emdash v${emdashVersion}" --body "${prBody.replace(/"/g, '\\"')}"`,
	{ cwd: targetDir },
);
